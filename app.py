...
def envoyer_email_reel(sujet, corps_html, destinataire=EMAIL_TEST_CIBLE):
    try:
        ...
        server.quit()
        st.toast(f"🚀 Email envoyé ! (Copie à {EMAIL_ADMIN})")
        return True
    except Exception as e:
...
def generer_pdf_html(cible_titre, df_print, print_ed, print_dr, print_px, print_prov, print_ipad, print_convention=False):
    ...
    for _, row in df_print.iterrows():
        ...
        # ==========================================
        if print_convention:
            # --- Paramétrage dynamique selon le niveau ---
    ...
    if res.empty and search_360:
        ...
        for _, el in res.iterrows():
            ...
            with st.expander(f"{status_icon} {el['prenom']} {el['nom']} ({el['classe']})", expanded=is_expanded):
                ...
                with tab_ipad:
                    # AFFICHAGE DU MODELE ET NUMERO DE SERIE
                    ...
                    # NOUVEAU SYSTEME SAV EN DIRECT (Sans st.form pour le calcul instantané)
                    ...
                    if btn_save_only or btn_save_mail:
                        ...
                        if btn_save_mail:
                            if prix_facture > 0:
                                ...
                                envoyer_email_reel(f"SAV iPad - {el['nom'].upper()} ({el['classe']})", corps_sav, EMAIL_TEST_CIBLE)
                            else:
        ...
        if up_jamf and st.button("🚀 Mettre à jour les numéros de série"):
            ...
            with st.spinner("Mise à jour en cours..."):
                for _, row in df_jamf.iterrows():
                    ...
                    # 💡 Cas spécifique demandé : "davout" sur le CSV correspond à "d'août" dans ta base
                    if n_clean == "davout":
                        n_clean = "daout"
